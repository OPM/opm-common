...
END
    ...
    const auto pressure_dim =
        unit_system.getDimension(UnitSystem::measure::pressure);

    // A well which has not been given a WELDRAW record carries no limit, but
    // does carry the pressure dimension: that is configured when the well is
    // created, not when the first record arrives.
    const auto& weldraw0 = sched.getWell("W1", 0).getWELDRAW();
    BOOST_CHECK(!weldraw0.active());
    BOOST_CHECK(WELDRAW{pressure_dim} == weldraw0);
