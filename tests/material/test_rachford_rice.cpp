...
BOOST_AUTO_TEST_CASE(CompositionScaleInvariant)
{
    const Dune::FieldVector<double, 3> K {5.0, 2.0, 0.2};
    const Dune::FieldVector<double, 3> z {0.2, 0.2, 0.6};
    const Dune::FieldVector<double, 3> scaledZ {2e-14, 2e-14, 6e-14};

    const auto liquidFraction = Opm::RachfordRice::solve(K, z);
    BOOST_CHECK_CLOSE_FRACTION(Opm::RachfordRice::solve(K, scaledZ), liquidFraction, 1e-12);
}

BOOST_AUTO_TEST_CASE(LargeEquilibriumRatio)
{
    const Dune::FieldVector<double, 2> K {1e200, 0.1};
    const Dune::FieldVector<double, 2> z {0.1, 0.9};

    BOOST_CHECK_CLOSE_FRACTION(Opm::RachfordRice::solve(K, z), 8.0 / 9.0, 1e-10);
}
