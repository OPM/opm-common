...
BOOST_AUTO_TEST_CASE(TestERft_2) {
    ...
    for (auto rft : rftList) {
        ...
        std::tuple<int, int, int> date=std::get<1>(rft);

        std::vector<std::tuple<std::string, EIOD::eclArrType, int>> arrayList = rft1.listOfRftArrays(wellName, date);
	    ...
	    std::string arrName=std::get<0>(array);
            ...
            eclTest.writeBinaryHeader(arrName,size,arrType);
