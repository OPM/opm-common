...
PAvgCalculatorCollection make_wbp(const EclipseState& es, const Schedule& sched) {
    PAvgCalculatorCollection wbp;
    for (const auto& wname : {"W_1", "W_2", "W_3", "W_4", "W_5", "W_6"}) {
        const auto report_step = sched.size() - 1;
        if (sched.hasWell(wname)) {
            PAvgCalculator calc(wname, 100, es.getInputGrid(), es.fieldProps().porv(true), sched.getWell(wname, report_step).getConnections(), PAvg());
            for (const auto& index : calc.index_list())
                calc.add_pressure(index, 100);
...
BOOST_AUTO_TEST_CASE(well_keywords) {
    ...
    writer.eval(st, 0, 0*day, cfg.wells, cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
    writer.add_timestep( st, 0);

    writer.eval(st, 1, 1*day, cfg.wells, cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
    writer.add_timestep( st, 1);

    writer.eval(st, 2, 2*day, cfg.wells, cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
...
BOOST_AUTO_TEST_CASE(DATE) {
    ...
    writer.eval( st, 2, 2 * day, cfg.wells , cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
    writer.add_timestep( st, 2);
    writer.eval( st, 3, 18 * day, cfg.wells , cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
    writer.add_timestep( st, 3);
    writer.eval( st, 4, 22 * day, cfg.wells , cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
...
BOOST_AUTO_TEST_CASE(report_steps_time) {
    ...
    out::Summary writer( cfg.es, cfg.config, cfg.grid, cfg.schedule, cfg.name );
    SummaryState st(TimeService::now());
    writer.eval( st, 1, 2 *  day, cfg.wells , cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
    writer.add_timestep( st, 1);
    writer.eval( st, 1, 5 *  day, cfg.wells , cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
    writer.add_timestep( st, 1);
    writer.eval( st, 2, 10 * day, cfg.wells , cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
    writer.add_timestep( st, 2);
    writer.write();
...
BOOST_AUTO_TEST_CASE(region_vars) {
...

    {
        out::Summary writer( cfg.es, cfg.config, cfg.grid, cfg.schedule, cfg.name );
        SummaryState st(TimeService::now());
        writer.eval( st, 1, 2 *  day, cfg.wells, cfg.grp_nwrk, {}, {}, {}, cfg.wbp, region_values);
        writer.add_timestep( st, 1);
        writer.eval( st, 1, 5 *  day, cfg.wells, cfg.grp_nwrk, {}, {}, {}, cfg.wbp, region_values);
        writer.add_timestep( st, 1);
        writer.eval( st, 2, 10 * day, cfg.wells, cfg.grp_nwrk, {}, {}, {}, cfg.wbp, region_values);
        writer.add_timestep( st, 2);
        writer.write();
    }
...
BOOST_AUTO_TEST_CASE(BLOCK_VARIABLES) {
    ...
    out::Summary writer( cfg.es, cfg.config, cfg.grid, cfg.schedule, cfg.name );
    SummaryState st(TimeService::now());
    writer.eval( st, 0, 0 * day, cfg.wells , cfg.grp_nwrk, {},{}, {}, cfg.wbp, {}, block_values);
    writer.add_timestep( st, 0);
    writer.eval( st, 1, 1 * day, cfg.wells , cfg.grp_nwrk, {},{}, {}, cfg.wbp, {}, block_values);
    writer.add_timestep( st, 1);
    writer.eval( st, 2, 2 * day, cfg.wells , cfg.grp_nwrk, {},{}, {}, cfg.wbp, {}, block_values);
    writer.add_timestep( st, 2);
    writer.eval( st, 3, 2 * day, cfg.wells , cfg.grp_nwrk, {},{}, {}, cfg.wbp, {}, block_values);
    writer.add_timestep( st, 3);
    writer.eval( st, 4, 2 * day, cfg.wells , cfg.grp_nwrk, {},{}, {}, cfg.wbp, {}, block_values);
    writer.add_timestep( st, 4);
    writer.write();
...
BOOST_AUTO_TEST_CASE(EXTRA) {
...

    {
        out::Summary writer( cfg.es, cfg.config, cfg.grid, cfg.schedule , cfg.name );
        SummaryState st(TimeService::now());
        writer.eval( st, 0, 0 * day, cfg.wells , cfg.grp_nwrk, { {"TCPU" , 0 }}, {}, {}, cfg.wbp);
        writer.add_timestep( st, 0);
        writer.eval( st, 1, 1 * day, cfg.wells , cfg.grp_nwrk, { {"TCPU" , 1 }}, {}, {}, cfg.wbp);
        writer.add_timestep( st, 1);
        writer.eval( st, 2, 2 * day, cfg.wells , cfg.grp_nwrk, { {"TCPU" , 2}}, {}, {}, cfg.wbp);
        writer.add_timestep( st, 2);

        /* Add a not-recognized key; that is OK */
        BOOST_CHECK_NO_THROW(  writer.eval( st, 3, 3 * day, cfg.wells , cfg.grp_nwrk, { {"MISSING" , 2 }}, {}, {}, cfg.wbp));
        BOOST_CHECK_NO_THROW(  writer.add_timestep( st, 3));

        /* Override a NOT MISC variable - ignored. */
        writer.eval( st, 4, 4 * day, cfg.wells, cfg.grp_nwrk, {}, {}, {}, cfg.wbp);
        writer.add_timestep( st, 4);
        writer.write();
    }
...
namespace {
    Opm::SummaryState calculateRestartVectors(const setup& config)
    {
      ...
      SummaryState st(TimeService::now());
      smry.eval(st, 0, 0*day, config.wells, config.grp_nwrk, {}, {}, {}, config.wbp);
      smry.add_timestep(st, 0);
      smry.eval(st, 1, 1*day, config.wells, config.grp_nwrk, {}, {}, {}, config.wbp);
      smry.add_timestep(st, 1);
      smry.eval(st, 2, 2*day, config.wells, config.grp_nwrk, {}, {}, {}, config.wbp);
...
BOOST_AUTO_TEST_CASE(Write_Read)
{
    ...
    SummaryState st(TimeService::now());
    writer.eval(st, 0, 0*day, config.wells, config.grp_nwrk, {}, {}, {}, config.wbp);
    writer.add_timestep(st, 0);
    writer.eval(st, 1, 1*day, config.wells, config.grp_nwrk, {}, {}, {}, config.wbp);
    writer.add_timestep(st, 1);
    writer.eval(st, 2, 2*day, config.wells, config.grp_nwrk, {}, {}, {}, config.wbp);
    writer.add_timestep(st, 2);
    writer.write();
