...
{{
    ...
    mutable std::unordered_map<std::string, ::Opm::ParserKeyword> keywords{};
        ...
        for (auto&& [first_char, source] : newSources) {
            ...
            write_file(
                source, sourceFile.generic_string(), m_verbose, fmt::format("builtin source for {}", first_char));
        }
    }
...
namespace Opm::ParserKeywords {{
    void addDefaultKeywords{0}(Parser& p);
            ...
            write_file(
                header, charHeaderFile.generic_string(), m_verbose, fmt::format("init header for {}", first_char));
...
{{
    // Built-in '{0}' keywords.
            ...
            write_file(
                sourceStr, charSourceFile.generic_string(), m_verbose, fmt::format("init source for {}", first_char));
