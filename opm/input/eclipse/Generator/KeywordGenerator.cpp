...
{{
    ...
    mutable std::unordered_map<std::string, ::Opm::ParserKeyword> keywords{};
        ...
        for (auto&& [first_char, source] : newSources) {
            const auto sourceFile = sourcePath / fmt::format("Builtin{}.cpp", first_char);
...
namespace Opm::ParserKeywords {{
    void addDefaultKeywords{0}(Parser& p);
            ...
            auto charHeaderFile = sourceFile;
...
{{
    // Built-in '{0}' keywords.
            ...
            const auto charSourceFile = sourcePath / fmt::format("ParserInit{}.cpp", first_char);
...
void python::common::export_ParserKeywords(py::module& module)
{
                                               ...
                                               const std::filesystem::path& sourcePath) const
    {
        for (const auto& [first_char, keywords] : loader) {
            ...
            write_file(newSource, sourcePath / fmt::format("{}.cpp", first_char),
                                        ...
                                        const std::filesystem::path& headerPath) const
    {
        for (const auto& [first_char, keywords] : loader) {
            ...
            const auto final_path = headerBuildPath / headerPath / fmt::format("{}.hpp", first_char);
    ...
    void KeywordGenerator::updateTest(const KeywordLoader& loader,
                                      const std::filesystem::path& testFile) const
    {
        std::stringstream stream;
