...
public:
    ...
    public:
        ...
        public:
            ...
            /// Iterators difference type
            using difference_type = int;

            /// Iterator's pointer type (return type from operator->())
            using pointer = int*;

            /// Iterators reference type (return type from operator*())
            using reference = int&;
    ...
    private:
        ...
        /// For use by RegionSetMatchResult only.
        ///
        /// \param[in] first Minimum region index value.
        ///
        /// \param[in] last One more than the maximum region index value.
        ///
        /// \param[in] region Name of region set to which this index range
        ///    is attached.
        RegionIndexRange(int first, int last, std::string_view region)
            : begin_  { first }
            , end_    { last }
            , region_ { region }
        {}
    ...
    bool isScalar() const
    {
        return (this->regionIDRange_.size() == std::vector<int>::size_type{2})
            && (this->regionIDRange_.front() == this->regionIDRange_.back());
    }
    ...
    /// Retrive result set's region indices for a single region set.
    ///
...
private:
    ...
    ///
    /// \param[in] minRegID Minimum region ID in match result for \p regSet.
    ///
    /// \param[in] maxRegID Maximum region ID in match result for \p regSet.
    ///   Must not be less than \p minRegID.
...
public:
    ...
    public:
        ...
        /// Retrive request's region number
        ///
        ...
        /// Retrive request's region set name
        ///
    ...
    /// Constructor
    ///
    /// \param[in] fipRegStats Input's notion of existing wells, both
