...
private:
    ...
    {
    public:
        RegIdxRange(const int min, const int max) : min_{min}, max_{max} {}

        int minRegID() const { return this->min_; }
        int maxRegID() const { return this->max_; }

        bool empty() const { return this->maxRegID() < this->minRegID(); }
                                         ...
                                         AddRegionSet         addRegions) const
{
    ...
    for (const auto& regSet : this->candidateRegionSets(request.regionSet())) {
        if (const auto regions = this->matchingRegions(regSet, request.regionID());
            ! regions.empty())
        {
            addRegions(regSet, regions.minRegID(), regions.maxRegID(), regSetMatchResult);
        }
    }
                ...
                const int          regionID) const
{
         ...
         (regionID <= this->fipRegStats_.get().declaredMaximumRegionID())))
    {
        ...
        // matching exactly that region ID.
        return { regionID, regionID };
    }

    // If we get here, 'regSet' does not exist or does not contain
    // 'regionID'.  Return empty range (max < min).
    return { 0, -1 };
}
...
Opm::RegionSetMatcher::Impl::matchingRegions(const std::string& regSet) const
{
    ...
    if (maxRegID <= 0) {
        // 'regSet' does not exist.  Return empty range (max < min).
        return { 0, -1 };
    }

    // 'regSet' is a valid region set name.  Return range covering 1..MAX.
    return { 1, std::max(maxRegID, this->fipRegStats_.get().declaredMaximumRegionID()) };
}
...
Opm::RegionSetMatchResult::regions(const std::size_t regSet) const
{
    ...
    const auto first = this->regionIDRange_[2*regSet + 0];
    const auto last  = this->regionIDRange_[2*regSet + 1];

    // Note: 'last + 1' is intentional here.  Last is the maximum region ID
    // in the match result, but the RegionIndexRange is a half-open interval
    // to mirror the usual "one-past-the-end" semantics of [begin, end).
    return { first, last + 1, this->regionSets_[regSet] };
}
...
void Opm::RegionSetMatchResult::addRegionIndices(const std::string& regSet,
                                                 ...
                                                 const int          maxRegID)
{
    assert (maxRegID >= minRegID);

    this->regionSets_.push_back(regSet);

    // Invariant: Push 'min' before 'max'.
    this->regionIDRange_.push_back(minRegID);
    this->regionIDRange_.push_back(maxRegID);
}
