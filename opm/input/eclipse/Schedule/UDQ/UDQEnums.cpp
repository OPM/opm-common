...
namespace { namespace UDAVersionCompat {
    ...
    RstFileUDAVersion udaVersion(const int rstFileVersion)
    {
...
UDAControl udaControl(const int uda_code, const RstFileUDAVersion rstFileUDAVersion)
{
    switch (rstFileUDAVersion) {
    ...
    default:
        throw std::invalid_argument {
            fmt::format("Restart file UDA version {} is unsupported",
                        static_cast<std::underlying_type_t<RstFileUDAVersion>>(rstFileUDAVersion))
        };
    }
}

UDAControl udaControl(const int uda_code, const int rstFileVersion)
{
    return udaControl(uda_code, UDAVersionCompat::udaVersion(rstFileVersion));
}
