...
Well {} is not connected to grid - will remain SHUT)",
            ...
            if (handlerContext.state().wells.get(name).isMultiSegment()) {
                ...
                for (size_t is = 0; is < intersections.size(); ++is) {
                    ...
                    intersections_md_and_ijk.emplace_back(intersections[is].startMD, intersections[is].endMD, ijk);
                    double cell_md = 0.5 * (intersections[is].startMD + intersections[is].endMD);
