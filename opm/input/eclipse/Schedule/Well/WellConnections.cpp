...
namespace Opm {
    ...
    {
        // Out-parameter: holds exactly the connections (re)opened by this
        // record, so clear any previous contents.
...
The cell ({},{},{}) in well {} is not active and the connection will be ignored)",
                                      ...
                                      std::vector<int>&      requested_open_complnums)
    {
        ...
        this->loadCOMPDATX(record, wname, wdfac, grid, location,
                           lgr_tag, parseContext, errors, requested_open_complnums);
    }
