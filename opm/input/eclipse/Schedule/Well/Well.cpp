...
Well::Well(const RestartIO::RstWell& rst_well,
    ...
    weldraw(std::make_shared<WELDRAW>(rst_well.max_drawdown,
                                      rst_well.wtype.preferred_phase(),
                                      unit_system_arg.getDimension(UnitSystem::measure::pressure))),
...
{
    // NOTE: well tracer facilities used for geochemistry
                                    ...
                                    std::vector<int>&       requested_shut_complnums)
{
    ...
    for (const auto& connection : *this->connections) {
        ...
        if (state_arg == Connection::State::OPEN) {
            ...
            // The connection is shut (or set to AUTO).  Report it so the caller
            // can clear any pending REQUEST_OPEN_COMPLETION event raised for the
            // same connection earlier.
            requested_shut_complnums.push_back(connection.complnum());
        }
