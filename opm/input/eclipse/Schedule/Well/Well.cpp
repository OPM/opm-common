                                            ...
                                            double diameter)
{
    if (this->segments != nullptr) {
        auto new_segments = std::make_shared<WellSegments>(*this->segments);
        new_segments->addWellSegmentsFromIntersections(this->name(), length_top, centers_md, centers_tvd, diameter, *unit_system);

        this->updateSegments(std::move(new_segments));
    }
    else {
        this->updateSegments(std::make_shared<WellSegments>(this->name(), length_top, centers_md, centers_tvd, diameter, *unit_system));
    }
}
