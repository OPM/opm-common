...
File {} line {}.)", pattern, location.keyword, location.filename, location.lineno);
    ...
    */
    ...
    SimulatorUpdate Schedule::runPyAction(std::size_t reportStep, const Action::PyAction& pyaction, Action::State& action_state, EclipseState& ecl_state, SummaryState& summary_state) {
        ...
        SimulatorUpdate simUpdateFromActionXCallback;
        auto apply_action_callback = [&simUpdateFromActionXCallback, &reportStep, this](const std::string& action_name, const std::vector<std::string>& matching_wells) {
            simUpdateFromActionXCallback = this->applyAction(reportStep, action_name, matching_wells);
        };
