...
namespace {
    ...
    operator()(const std::vector<int>& controlValues) const
    {
        ...
        // adding/updating the integer list sourced BASIC mnemonic, should
        // it be zero.  I'm not sure if this applies to other mnemonics, but
        // the ECLIPSE manual indicates that any zero here should disable
        // the output.
        //
        ...
        // keywords is probably not correct, but it is extremely difficult
        // to comprehend exactly how it should be. Current code is a rather
        // arbitrary hack to get through the tests.

        if (numValues >= PCO_index) {
            for (std::size_t i = 0; i < PCO_index; ++i) {
                mnemonics.insert_or_assign(this->keywords_[i], controlValues[i]);
            }
        }
        else {
            if ((numValues > 0) && (controlValues[BASIC_index] != 0)) {
                mnemonics.insert_or_assign(this->keywords_[BASIC_index],
                                           controlValues  [BASIC_index]);
            }

            for (auto i = 1 + 0*numValues; i < numValues; ++i) {
                mnemonics.insert_or_assign(this->keywords_[i], controlValues[i]);
            }
        }
        ...
        if (controlValues.size() > PCO_index) {
            mnemonics.insert_or_assign("PCOW", controlValues[PCO_index]);
            mnemonics.insert_or_assign("PCOG", controlValues[PCO_index]);
        }
    ...
    {
        auto mnemonics = Opm::RPTKeywordNormalisation {
            RptRstIntegerControlHandler{},
                             ...
                             ErrorGuard&         errors)
{
    ...
    auto mnemonics = normaliseRptSchedKeyword(keyword, parseContext, errors);
