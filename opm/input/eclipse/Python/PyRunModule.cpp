...
PyRunModule::PyRunModule(std::shared_ptr<const Python> python, const std::string& fname) {
    ...
    try {
        ...
        std::cerr << "Exception thrown when loading Python module opm_embedded: " << e.what() << std::endl;
        throw e;
    } catch (...) {
        std::cerr << "General exception thrown when loading Python module opm_embedded!" << std::endl;
    }
}
...
bool PyRunModule::executeInnerRunFunction(const std::function<void(const std::string&, const std::vector<std::string>&)>& actionx_callback) {
    ...
    try {
        ...
        try {
            py::object result = this->run_function(this->opm_embedded.attr("current_ecl_state"), this->opm_embedded.attr("current_schedule"), this->opm_embedded.attr("current_report_step"), this->opm_embedded.attr("current_summary_state"));
            return result.cast<bool>();
        } catch (const std::exception& e2) {
            std::cerr << "Exception thrown trying to call run(ecl_state, schedule, report_step, summary_state, actionx_callback) function of " << this->module_name << ": " << e.what() << std::endl;
            std::cerr << "Tried calling run(ecl_state, schedule, report_step, summary_state), exception thrown: " << e2.what() << std::endl;
            throw e2;
        }
    }
}
...
bool PyRunModule::run(EclipseState& ecl_state, Schedule& sched, std::size_t report_step, SummaryState& st, const std::function<void(const std::string&, const std::vector<std::string>&)>& actionx_callback) {
    ...
    if (!this->module) {
        ...
        try {
            ...
            std::cerr << "Exception thrown when loading Python module " << this->module_name << ": " << e.what() << std::endl;
            throw e;
        } catch (...) {
            std::cerr << "General exception thrown when loading Python module " << this->module_name << "!" << std::endl;
        }

        if (this->module.is_none()) {
            throw std::runtime_error("Syntax error when loading Python module: " + this->module_name);
        }

        this->module.attr("storage") = this->storage;

        if (py::hasattr(this->module, "run")) {
            std::cout << R"(PYACTION can be used without a run(ecl_state, schedule, report_step, summary_state, actionx_callback) function, its arguments are available as attributes of the module opm_embedded, try the following in your python script:
