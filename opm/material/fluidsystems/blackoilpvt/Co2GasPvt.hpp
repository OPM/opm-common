...
private:
                    ...
                    const SaltArray<LhsEval, SaltMassFraction>& salinity) const
    {
        ...
        if (enableMultiCompSalt_) {
            ...
            LhsEval M_Brine =
                Brine::avgMolarMassFromMoleFrac(salinity.template convert_to<SaltMoleFraction>());
            LhsEval avgMolarMass = M_CO2 + xgW * (M_Brine - M_CO2);

            return xgW * M_Brine / avgMolarMass;
        }
...
namespace Opm::gpuistl {
    ...
    make_view(Co2GasPvt<ScalarT, GpuBuffer>& co2GasPvt)
    {
        ...
        auto newSalinity = make_view<SaltContainedType>(co2GasPvt.salinity_);
