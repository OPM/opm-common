 ...
 * T0 = IdealGasCaloricData::referenceTemperature(): every component's
 * ideal-gas enthalpy is zero there, at any pressure, so enthalpies of
 * different mixtures share one zero and add up. The caloric model gives
 * H(T0) = 0. The eos_departure model gives H(T0) = the residual enthalpy of
 * the flashed state at (T0, P), which is not zero; it vanishes only as P -> 0.
 *
 * Derivatives: the caloric overloads are generic in the fluid state's value
 * type, and their derivatives are complete. The model-switch overloads return
...
struct MixtureEnthalpy {
     ...
     *        model == eos_departure.
     *
     * Returns a plain Scalar for any fluid-state value type. The departure
     * part carries no derivatives (see phaseResidualEnthalpy), so an AD
     * result would hold a derivative that looks complete but is not. For
     * derivatives of the caloric model, use the caloric overload.
     */
    template <class FluidState>
    static Scalar
    phaseEnthalpy(const FluidState& fluidState,
    ...
    {
        Scalar h = Opm::getValue(phaseEnthalpy(fluidState, phaseIdx, cpTable, refTemperature));
                    ...
                    const EnthalpyModel model)
    {
        ...
        Scalar hOil = Opm::getValue(phaseEnthalpy(fluidState, FluidSystem::oilPhaseIdx, cpTable, refTemperature));
        Scalar hGas = Opm::getValue(phaseEnthalpy(fluidState, FluidSystem::gasPhaseIdx, cpTable, refTemperature));
        if (model == EnthalpyModel::eos_departure) {
            if (L > 0.0)
