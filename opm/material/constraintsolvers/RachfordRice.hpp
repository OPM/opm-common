...
private:
    ...
    static typename Vector::field_type validateInputs(const Vector& K, const Vector& z)
    {
        if (K.size() == 0) {
    ...
    static typename Vector::field_type g(const Vector& K,
                                         ...
                                         const Vector& z,
                                         typename Vector::field_type compositionTotal)
    {
        ...
        for (std::size_t compIdx = 0; compIdx < K.size(); ++compIdx) {
            ...
            if (L == 0) {
                ...
                ratio = dK / (K[compIdx] - L * dK);
            }
            value += (z[compIdx] / compositionTotal) * ratio;
        }
        return value;
    }
                                                 ...
                                                 int verbosity)
    {
        ...
        typename Vector::field_type gLmin = g(K, Lmin, z, compositionTotal);
        ...
        for (int iteration = 0; iteration < max_it; ++iteration) {
            ...
            auto gMid = g(K, L, z, compositionTotal);
...
public:
    ...
    {
        using field_type = typename Vector::field_type;
        const field_type compositionTotal = validateInputs(K, z);
        ...
        if (g(K, field_type {1}, z, compositionTotal) <= 0) {
            return field_type {1};
        }
        if (g(K, field_type {0}, z, compositionTotal) >= 0) {
            return field_type {0};
        }
        ...
        for (int iteration = 1; iteration < itmax; ++iteration) {
            ...
            for (std::size_t compIdx = 0; compIdx < K.size(); ++compIdx) {
                ...
                const auto normalizedZ = z[compIdx] / compositionTotal;
                const auto dK = K[compIdx] - 1.0;
                const auto b = 1 + V * dK;
                const auto dKOverB = dK / b;
                r += normalizedZ * dKOverB;
                denum += normalizedZ * dKOverB * dKOverB;
            }
            auto delta = r / denum;
            V += delta;
            ...
            if (V < Vmin || V > Vmax) {
                ...
                field_type Lmin = 0;
                field_type Lmax = 1;
                auto L = bisection(K, Lmin, Lmax, z, compositionTotal, verbosity);
