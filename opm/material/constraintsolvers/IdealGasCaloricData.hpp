 ...
 * molar enthalpy [J/mol]. The datum is the ideal gas at the reference
 * temperature: every component's ideal-gas enthalpy is zero there, at any
 * pressure.
 */
 ...
 *
 * The datum is fixed ONCE for the whole enthalpy stack: the ideal gas at the
 * reference temperature T0, where every component's ideal-gas enthalpy is
 * zero. Ideal-gas enthalpy does not depend on pressure, so the datum has no
 * reference pressure. Under the caloric model a mixture therefore has
 * H(T0) = 0; under the EoS-departure model H(T0) is the residual enthalpy of
 * the state, which is not zero (see MixtureEnthalpy). Any specified enthalpy
 * H_spec handed to the isenthalpic flash must be expressed against this
 * datum, i.e. computed with the same model.
 */
template <class Scalar>
struct IdealGasCaloricData {
    //! reference temperature T0 [K]; every component's ideal-gas enthalpy is zero here
