...
class Common
{
    // Source values in double precision; the public constants and the ones
    // derived from them (Rs, criticalMolarVolume) all come from these, so a
    // correction to one cannot leave the other stale.
