...
namespace Opm::gpuistl {
    ...
    make_view(UniformTabulated2DFunction<ScalarT, GpuBuffer>& tab) {
        auto newTab = make_view<ScalarT>(tab.samples_);
        return UniformTabulated2DFunction<ScalarT, GpuView>(tab.xMin(), tab.xMax(), tab.numX(), tab.yMin(), tab.yMax(), tab.numY(), newTab);
    }
} // namespace Opm::gpuistl
