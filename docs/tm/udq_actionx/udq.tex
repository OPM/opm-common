...
define arithmetic expressions based on the result vectors from of the ongoing
...
subcommands: \kw{ASSIGN}, \kw{DEFINE}, \kw{UNIT} and \kw{UPDATE}. The
\kw{ASSIGN} subcommand is used to define a \udq{} variable with a constant
numerical value. \kw{DEFINE} is used to define an arithmetic expression for a
\udq{} variable, this expression is evaluated at the end of every simulator time
step. The \kw{UNIT} command is used to assign a unit string to a \udq{}
variable, see section \ref{udq_units} for more details about \udq{} variables and
units. The \kw{UPDATE ON} and \kw{UPDATE OFF} commands can be used to switch
updating of \udq{} variables on and off. The \udq{} variables are created in
mode \kw{ON}.
...
\inlinecode{FU1 = FWPR}, i.e. the initial definition \inlinecode{FU1 = 100} has
...
/
\end{deck}
we will define one scalar keyword \kw{FU1}, one well set \kw{WU1} and one
group set \kw{GU1}. The well sets will have one slot for each well in the model,
...
maintains a \inlinecode{is\_defined()} status and most operations
...
for each well, and \kw{WUOPR} which is the oil production rate for a subset of
wells:
\begin{deck}
UDQ
  DEFINE WUBHP  WBHP     / 
...
given as item 3 of the \kw{UDQPARAM} keyword, observe that summary output is the
\emph{only} point where the undefined value can be dereferenced, the numerical
value given in \kw{UDQPARAM} will not be available either for \udq{} nor
\kw{ACTIONX} evaluations.
...
The following combination of \udq{} and \kw{ACTIONX} will use the \kw{SORTD}
...
\item The \udq{} variable \kw{WUI} is evaluated as the water injection rate from
  \inlinecode{SummaryState} subtracted 100.
\end{enumerate}

Observe that the units are slightly complicated, and non intuitive for \kw{UDA}
