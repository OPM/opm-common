...
parsed when the simulator starts, and then the well \kw{W1} is unknown in the
\kw{WCONPROD} keyword. This is ``solved'' in the following way:
\begin{enumerate}
...
keywords in the \kw{SCHEDULE} section where the implementation is not yet
...
\inlinecode{Action::WGNames action\_wgnames} in the \inlinecode{Schedule} class.
...
will manage an instance of \inlinecode{Action::State} which will hold on to the
...
copying information from the internals of the \inlinecode{Schedule} object. Some
...
to open or close a well or \kw{WCONPROD} to adjust rates the simulator must
...
\subsubsection{WELPI}
The \kw{WELPI} keyword is quite complex anyhow, when it is included as an
\actionx{} keyword it gets even more complex. In the case the \actionx{} keyword
contains \kw{WELPI} the simulator must first fetch the PI values and pass them
to the \inlinecode{Schedule::applyAction()} function.
