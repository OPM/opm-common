...
\section{Changing the \inlinecode{Schedule} object}
\label{pyaction_actionx}
Before reading this section you should make sure to understand the
\inlinecode{Schedule} design described in section \ref{schedule_design}.
We can make function calls like
\begin{code}
  schedule.close_well(w1, report_step)
  schedule.set_orat(w2, 1000, report_step)
\end{code}
to close a well and set the oil rate of another well. Interally, this is done by
creating an \kw{ACTIONX} keyword for each call that changes the
\inlinecode{Schedule} object.
...
condition applies. In the example below we close all wells which have
