...
public:
    ...
    void set_outlet(std::shared_ptr<Node> outlet) { m_outlet = outlet; };
    std::shared_ptr<Node> get_outlet() { return m_outlet; };

    void add_inlet_node(std::shared_ptr<Node> n);

    void set_vfp(int vfp){m_vfp = vfp;};
    int get_vfp(){ return m_vfp;};

    void set_fixed_pres(double pres){m_fixed_pres = pres; };
    double get_fixed_pres(){return m_fixed_pres; };

    int get_xpos(){return m_xpos;};

    void print(std::stringstream& netw_str);

    bool detete_from_inlet_list(const std::string& name);

    bool add_well(const std::string& name);

    void reset_outlet(){m_outlet = nullptr;};
...
std::shared_ptr<Node> Node::next_branch(){
...
bool Node::detete_from_inlet_list(const std::string& name){
...
bool Node::add_well(const std::string& name){
...
void Node::print(std::stringstream& netw_str){
...
void Node::add_inlet_node(std::shared_ptr<Node> node)
{
    for (size_t n = 0; n < m_inlet_list.size(); n ++ ){
        if (m_inlet_list[n]->name() == node->name()){
            m_inlet_list.erase (m_inlet_list.begin() + n);
            break;
        }
    }
...
public:
    ...
    int number_report_steps() {return m_report_time_list.size();}
...
void NetWork::parse_data_deck(const std::filesystem::path& inputFileName){
            ...
            for (auto& rec : keyw ){
                ...
                if (m_report_time_list.size() == 0){
                    if ((last_time <=  m_start_date) && (!skiprest)) {
                        std::cout << "\n!Error, next report step '" << time_str(last_time) << "' has already passed \n\n";
                        exit(1);
                    }
                } else {
...
void NetWork::parse_unrst(const std::filesystem::path& inputFileName){
...
std::string NetWork::time_str(time_t t1){

    std::vector<std::string> mndStr{"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};
...
void NetWork::print_network_input(){
...
bool NetWork::node_exist(const std::string& name)
{
...
void NetWork::add_node(const std::string& name)
{
...
void  NetWork::add_branch(const std::string& downtree, const std::string& uptree, int vfp)
{
    if (vfp == 0){
...
void  NetWork::delete_branch(const std::string& downtree, const std::string& uptree)
{
    std::shared_ptr<Node> pUptree;
    std::shared_ptr<Node> pDowntree;
    ...
    if (pUptree->detete_from_inlet_list(downtree) == false)  {
...
void  NetWork::build_network(int rstep)
{
...
void NetWork::print_network(int rstep){
...
void NetWork::print_report_steps(){
...
time_t NetWork::time_from_rec(const Opm::DeckRecord& rec){

    std::map<std::string, int> mnd_map{{"JAN", 0}, {"FEB", 1}, {"MAR", 2}, {"APR", 3}, {"MAY", 4}, {"JUN", 5},
                                       {"JUL", 6}, {"JLY", 6}, {"AUG", 7}, {"SEP", 8}, {"OCT", 9}, {"NOV", 10},
                                       {"DEC", 11}};
...
time_t NetWork::time_from_rst(const std::string& rstfile, int rstep){
    ...
    std::tm date1 = {sec, min, h, day, mnd - 1, year - 1900 };

    return std::mktime(&date1);
}

void NetWork::br_input_from_rst(const std::string& rstfile, std::vector<int> rstep_vect){
...
static void printHelp() {
    ...
    std::cout << "\n This program visualizes a production network with termial output."
              << " Input to this program should be a valied data deck (.DATA) \n or a unified"
              << " restart file (.UNRST).\n\n The program takes these options"
              << " (which must be given before the arguments):\n\n"
              << " -l lists all available report steps and exit.\n"
              << " -r selets report step to be visualized. Default is the last report step \n"
