                 ...
                 const EclipseState& es, const time_t start_time)
{
    ...
    outFile.write("KEYCHECK", m_smry_keys, 24);
    outFile.write("UNITS", m_smryUnits);

    uint64_t rstep_pos = outFile.file_pos();

    std::vector<int> tmp(m_maxTimeSteps, -1);
    m_rstep = std::move(tmp);

    std::vector<float> zero_vect(m_maxTimeSteps, 0.0);

    m_smrydata.reserve(m_nVect);

    for (size_t n=0; n < m_nVect; n++)
        m_smrydata.push_back(zero_vect);
...
void LODSmryOutput::write(const std::vector<float>& ts_data, int report_step)
{
    ...
    std::chrono::duration<double> elapsed_since_last_write = lap0 - m_end_last_write;
    ...
    m_rstep[m_nTimeSteps] = report_step;

    for (size_t n = 0; n < static_cast<size_t>(m_nVect); n++)
        m_smrydata[n][m_nTimeSteps] = ts_data[n];
    ...
    std::cout << " Writing LODSMRY:  step " << m_nTimeSteps << "/" << m_maxTimeSteps;
    std::cout << "   + " << std::setw(8) << std::setprecision(5) << std::fixed << elapsed_wts.count();
    std::cout << " s,  total writing : " << m_elapsed_writing << " s ";
    std::cout << "  time since last write: " << elapsed_since_last_write.count();
    std::cout << " s \n" << std::flush;

    m_nTimeSteps++;
    m_end_last_write = std::chrono::system_clock::now();
}
...
int LODSmryOutput::expand_and_rewrite(int new_size)
{
    ...
    */
    ...
    size_t incSize = new_size - m_rstep.size();

    if (incSize > 100)
        incSize = 100;

    new_size = m_maxTimeSteps + incSize;

    std::vector<int> inc_rstep(incSize, -1);
    std::vector<float> inc_values(incSize, 0.0);

    m_rstep.insert(m_rstep.end(), std::begin(inc_rstep), std::end(inc_rstep));

    for (size_t n=0; n < static_cast<size_t>(m_nVect); n++){
        m_smrydata[n].insert(m_smrydata[n].end(), std::begin(inc_values), std::end(inc_values));
    }
...
void LODSmryOutput::write(const std::vector<float>& ts_data, int report_step)
{
    ...
    if (m_nTimeSteps == m_maxTimeSteps) {
        std::cout << "expanding in progress, stopping ? \n";
        exit(1);
        m_maxTimeSteps = this->expand_and_rewrite(m_nTimeSteps*2);
    }
