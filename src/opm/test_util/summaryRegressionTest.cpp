...
void RegressionTest::getRegressionTest(const char* keyword){
    ...
    OPM_THROW(std::invalid_argument, "Input keyword from user does not exist in/is not common for the two summary files.");
}



void RegressionTest::checkDeviation(Deviation deviation, const char* keyword, int reportStep){
    double absTol = getAbsTolerance();
    double relTol = getRelTolerance();

    if (deviation.rel > relTol && deviation.abs > absTol){
        std::cout << "For keyword " << keyword << " at reportstep " << reportStep <<std::endl;
        std::cout << "(days, reference value) and (days, check value) = (" << (*referenceVec)[reportStep] << ", " << (*referenceDataVec)[reportStep]
            << ") and (" << (*checkVec)[reportStep] << ", " << (*checkDataVec)[reportStep] << ")\n";
        ...
        OPM_THROW(std::invalid_argument, "Deviation exceed the limit.");
    }
}
...
void RegressionTest::startTest(const char* keyword){
    ...
    for (size_t ivar = 0; ivar < referenceVec->size(); ivar++){
        ...
        checkDeviation(deviation, keyword,ivar);
    }
}
