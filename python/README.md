...
- To enable tooltips for embedded Python code (opm_embedded) for the editor VS Code, first make sure the stub file in `<opm-common-folder>/python/opm_embedded/__init__.pyi` is updated!
  ...
  Then:
    - Either: Build the moodule opm-common and install it with "make install" - this will copy the file `<opm-common-folder>/python/opm_embedded/__init__.pyi` to the loction where opm_embedded is installed, making it visible to VSCode.
